...
const getProduct = (title) => products.find((product) => product.title === title);
const getPrinter = (id) => printers.find((printer) => printer.id === id);
...
const saveDesignsToStorage = () => {
    const savedDesigns = products.filter((product) => product.author);
...
if (app) {
  ...
  `;
    ...
    const renderDesignDetailPage = (productId) => `
    ${getHeaderHTML()}
  ...
  `;
    ...
    const attachEventListeners = () => {
        ...
        if (buyerForm) {
            buyerForm.addEventListener('submit', (e) => {
                ...
                const pass = document.getElementById('buyerPassword').value;
        ...
        const creatorForm = document.getElementById('creatorLoginForm');
        if (creatorForm) {
    ...
    const setupGallery = () => {
        ...
        const popularListEl = document.querySelector('#popularList');
        if (popularListEl) {
